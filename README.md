...
Pour la base de données, créez une nouvelle base MySQL. Mettez à jour la valeur de `DATABASE_URL` avec vos informations dans le fichier `.env` du dossier `Back`.  
Ensuite exécutez la commande `npx prisma migrate dev --firstmig init`.  
Enfin installez le client Prisma avec la commande `npm install @prisma/client`.  
